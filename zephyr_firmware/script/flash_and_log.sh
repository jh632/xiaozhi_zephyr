...
#
# 使用前请自行激活 west 环境(提供 west 与 pyserial), 脚本不会修改环境变量。
#
# 用法: script/flash_and_log.sh [抓取秒数] [串口设备] [west flash 的额外参数...]
#   抓取秒数  默认 20 秒
#   串口设备  默认依次探测各 USB 串口, 取第一个复位后有输出的端口
#   其余参数  原样传给 west flash, 用于指定烧录器或调试器
#
...
if [[ -z "${PORT}" ]]; then
	...
	PORT="$(python3 "${SCRIPT_DIR}/python/find_port.py")" || {
		echo "错误: 没有探测到串口, 请把串口设备作为第二个参数传入" >&2
		exit 1
	}
fi
