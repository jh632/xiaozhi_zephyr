...
`src/main.c` 先打印板级信息并扫描 I2C 总线，核对引脚分配与硬件是否一致，
然后跑一遍音频自检。I2C 总线上应能看到：
...
`src/main.c` 的音频自检按顺序做三件事，并打印每段的 RMS、峰值与 440 Hz 分量幅度：
录 1 秒环境噪声、播 2 秒 440 Hz 正弦（0 dB）、再播 2 秒（-20 dB）。
播放期间用板载麦克风回采，回采信号里 440 Hz 分量的幅度用 Goertzel 算法在测试音频率上
单点测得，它随输出音量变化，因此放音是否真的出声、音量属性是否生效都可以从串口日志核对。
