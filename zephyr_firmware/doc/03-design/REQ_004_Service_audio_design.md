...
模块内部负责 Opus 编解码、声道提取、任务调度与队列管理。application 层只做设备状态判断与协议收发，不直接接触 I2S 与编解码器。

接口划分与队列语义参考上游 xiaozhi-esp32 的 `main/audio/audio_service.h`，按本板的硬件能力调整。
...
| 硬件 | ES8311 编解码器、ZTS6216 麦克风、NS4150B 功放与扬声器 |

音频 service 通过 Zephyr 的 audio codec 接口访问编解码器，器件句柄来自设备树，更换板上的编解码器芯片不需要改动本模块。本板使用 ES8311，Zephyr 没有提供它的驱动，由本工程实现。

音频参数全链路固定为 16 kHz、16 位，单声道数据放在两个 I2S 槽位里。Opus 帧长 60 毫秒，与服务器协议一致。
...
I2S 配置顺序固定为先播放方向、再采集方向。ESP32-C3 的收发共用同一组 WS/BCK，播放方向先配置之后，采集方向作为从机使用同一套帧同步。
...
| 约束 | 对设计的影响 |
|---|---|
| ESP32-C3 单核 160 MHz、400 KB SRAM、无 PSRAM | 任务、栈、队列全部静态分配；Opus 编码使用较低复杂度参数 |
| 不支持 ESP-SR 的音频前端（AFE） | 设备端回声消除与前端降噪不可用，回声消除由服务器承担 |
| 唤醒词模型与推理库来自 ESP-SR | 在 Zephyr 下需要单独引入，接口按模型可用的前提设计 |
| 全链路固定 16 kHz | 不需要重采样组件，编解码器与 I2S 采样率始终一致 |
| I2S 收发共用 WS/BCK | 配置顺序固定为播放方向在先 |
| 功放使能 GPIO13 没有下拉电阻 | 播放开始前由 regulator 驱动保持低电平，扬声器静默 |
...
| 唤醒词推理与模型 | 唤醒词检测 | 模型与推理库来自 ESP-SR，需要单独引入 |
